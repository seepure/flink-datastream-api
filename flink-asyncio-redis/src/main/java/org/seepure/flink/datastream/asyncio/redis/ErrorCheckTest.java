...
import org.seepure.flink.datastream.asyncio.redis.SimpleBatchIOJob.SimpleRedisBatchFlatMap;
import org.seepure.flink.datastream.asyncio.redis.SimpleFlatMapJob.SimpleRedisFlatMap;
import org.seepure.flink.datastream.asyncio.redis.sink.ErrorCheckerSink;
...
public class ErrorCheckTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String defaultRedisHashJoinArg =
                "redis.mode=cluster;redis.nodes=redis://192.168.213.128:7000,redis://192.168.213.128:7001,redis://192.168.213.129:7000,redis://192.168.213.129:7001,redis://192.168.213.130:7000,redis://192.168.213.130:7001"
                        + ";source.schema.type=MQ_KV;source.schema.content={};dim.schema.type=redis.hash;dim.schema.content={};joinRule.rightFields=kh_%s;"
                        + "cachePolicy.type=local;cachePolicy.expireAfterWrite=300;cachePolicy.size=9000";
        ...
        String keyColumn = "mykey";
        int bound = 100_000;
        DataStream<String> in = env.addSource(new PressureRandomSource(keyColumn, bound, 1000));
        SingleOutputStreamOperator<String> stream = in.flatMap(new SimpleRedisBatchFlatMap(configMap));
        stream.addSink(new ErrorCheckerSink(keyColumn, bound, Integer.parseInt(configMap.getOrDefault("cachePolicy.expireAfterWrite", "30"))));
        env.execute();
    }
